...
CREATE TABLE CALENDRIER1(
   ...
   CONSTRAINT CHK_CAL1_DATEFAB_LE_SYSDATE CHECK (DATEFAB <= TRUNC(CURRENT_DATE))
);
...
CREATE TABLE CALENDRIER3(
   ...
   CONSTRAINT CHK_CAL3_ANNE_POSITIVE CHECK (ANNEE <= to_char(CURRENT_DATE, 'YYYY')),
   CONSTRAINT CHK_CAL3_MOIS_POSITIVE CHECK (MOIS >= 1 AND MOIS <= 12)
);

CREATE TABLE CALENDRIER4(
   ANNEE NUMBER(4) PRIMARY KEY
   CONSTRAINT CHK_CAL4_ANNE_POSITIVE CHECK (ANNEE <= to_char(CURRENT_DATE, 'YYYY'))
);
